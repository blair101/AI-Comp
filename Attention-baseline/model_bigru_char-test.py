...
data = pd.read_csv("data/preprocess/train_char.csv")
data["content"] = data.apply(lambda x: eval(x[1]), axis=1)

validation = pd.read_csv("data/preprocess/validation_char.csv")
...
print(len(word_index)) # 7934

# print(embeddings_matrix)
# print(type(embeddings_matrix))# <class 'numpy.ndarray'>

print(w2_model.vector_size) # 100
print(word_index) # {'不': 1, '很': 2, '吃': 3, ...}

word2idx = {"_PAD": 0}

vocab_list = [(k, w2_model.wv[k]) for k, v in w2_model.wv.vocab.items()]

# print(vocab_list[0]) 不
# print(vocab_list[1]) 很 ('很', array([100维度])), ...
# print(len(vocab_list)) 7983 个字。
